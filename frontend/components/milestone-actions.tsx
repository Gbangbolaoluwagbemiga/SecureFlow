...
export function MilestoneActions({
  ...
  const [actionType, setActionType] = useState<"start" | "submit" | "approve" | "dispute" | "resolve" | null>(null)
    ...
    try {
      ...
      switch (actionType) {
        case "start":
          ...
          break
        case "submit":
          txHash = await contract.send("submitMilestone", escrowId, milestoneIndex)
          toast({
          ...
          break
        case "approve":
          txHash = await contract.send("approveMilestone", escrowId, milestoneIndex)
          toast({
  ...
  const getDialogContent = () => {
    switch (actionType) {
      case "start":
        return {
          title: "Start Work",
          description: "Confirm that you're ready to start working on this project. This will activate the escrow.",
          icon: Play,
      ...
      case "resolve":
        return {
          ...
          description: "As the contract owner, you can resolve this dispute in favor of either party.",
          icon: Gavel,
          confirmText: "Resolve",
        }
      default:
    ...
    <>
      <div className="flex flex-wrap gap-2">
        ...
        {escrowStatus === "pending" && isBeneficiary && (
          <Button onClick={() => openDialog("start")} size="sm" className="gap-2">
        ...
        {milestone.status === "pending" && isBeneficiary && escrowStatus === "active" && (
          <Button onClick={() => openDialog("submit")} size="sm" variant="default" className="gap-2">
        ...
        {milestone.status === "submitted" && isPayer && (
          <Button onClick={() => openDialog("approve")} size="sm" variant="default" className="gap-2">
        ...
        {(milestone.status === "submitted" || escrowStatus === "active") && (isPayer || isBeneficiary) && (
          <Button onClick={() => openDialog("dispute")} size="sm" variant="destructive" className="gap-2">
